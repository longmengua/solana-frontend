...
export async function unlockNFT(connection: Connection, wallet: AnchorWallet, nftAccount: PublicKey, nftAddress: PublicKey): Promise<{
  tx?: string,
  error?: any,
}> {
  ...
  try {
    const txId = await program.methods.unlock(
      configNonce,
      exchangeBump,
